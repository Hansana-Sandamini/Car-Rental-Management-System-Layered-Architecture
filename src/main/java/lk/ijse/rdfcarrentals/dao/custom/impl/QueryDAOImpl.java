...
public class QueryDAOImpl implements QueryDAO {
    ...
    @Override
    public ObservableList<Double> getIncomeMonthly(int year) throws SQLException, ClassNotFoundException {
    ...
    @Override
    public int getYearTotalSaleAmount(int year) throws SQLException, ClassNotFoundException {
        ResultSet rst = SQLUtil.execute(
                "SELECT MONTH(r.pick_up_date) AS MONTH, " +
                        "COALESCE(SUM(p.amount), 0) + COALESCE(SUM(CASE WHEN c.amount_to_pay = 0 THEN c.total_amount ELSE 0 END), 0) AS monthly_income " +
                        "FROM reservation r " +
